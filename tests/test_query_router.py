...
def test_route_skips_llm_when_no_communities(monkeypatch):
    ...
    monkeypatch.setattr("local_graph_rag.rag.query_router.ollama_client.generate", _record)
...
def test_route_uses_llm_global_response(monkeypatch):
    monkeypatch.setattr(
        "local_graph_rag.rag.query_router.ollama_client.generate",
        lambda *a, **kw: "global",
    )
...
def test_route_falls_back_to_heuristic_on_llm_failure(monkeypatch):
    ...
    monkeypatch.setattr("local_graph_rag.rag.query_router.ollama_client.generate", _raise)
