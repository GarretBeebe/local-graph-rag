...
class GraphStore:
    ...
    def upsert_entity(
        ...
        with self._write():
            ...
            if existing is None:
                ...
                merged_desc = max(existing["description"] or "", description or "", key=len)
                self._conn.execute(
    ...
    def register_chunk(self, chunk_id: str, filepath: str, chunk_index: int) -> None:
        with self._write():
            self._conn.execute(
                "INSERT OR REPLACE INTO chunks (chunk_id, filepath, chunk_index) VALUES (?, ?, ?)",
                (chunk_id, filepath, chunk_index),
            )
    ...
    def link_chunk(self, chunk_id: str, entity_ids: list[str]) -> None:
        with self._write():
